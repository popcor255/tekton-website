...
- Optional:
  ...
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/pipelines-v0.11.3/resources) needed or created by your`Task`.
...
- The container image must abide by the [container contract](/vault/pipelines-v0.11.3/./container-contract).
...
a `[`PipelineResources`](/vault/pipelines-v0.11.3/resources#using-resources) entity.
...
For more information, see [Using `Workspaces` in `Tasks`](/vault/pipelines-v0.11.3/workspaces#using-workspaces-in-tasks)
...
- [Mount a Kubernetes `Secret`](/vault/pipelines-v0.11.3/auth).
...
For further information, see [`Sidecars` in `TaskRuns`](/vault/pipelines-v0.11.3/taskruns#sidecars).
...
Also see the [complete list of variable substitutions for Tasks](/vault/pipelines-v0.11.3/./variables#variables-available-in-a-task).
...
- To reference a parameter in a `Task`, use the following syntax, where `<name>` is the name of the parameter:
  ...
  $(params.<name>)
  ```
- To access parameter values from resources, see [variable substitution](/vault/pipelines-v0.11.3/resources#variable-substitution)
