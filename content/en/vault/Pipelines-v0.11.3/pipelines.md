...
- Optional:
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/pipelines-v0.11.3/resources) needed or created by the `Tasks` comprising the `Pipeline`.
  - `tasks`:
      - `resources.inputs` / `resource.outputs`
        - [`from`](#using-the-from-parameter) - Indicates the data for a [`PipelineResource`](/vault/pipelines-v0.11.3/resources)
...
A `Pipeline` requires [`PipelineResources`](/vault/pipelines-v0.11.3/resources) to provide inputs and store outputs
...
- [Using `Workspaces` in `Pipelines`](/vault/pipelines-v0.11.3/workspaces#using-workspaces-in-pipelines)
...
by using [variable substitution](/vault/pipelines-v0.11.3/variables#variables-available-in-a-pipeline).
...
## Adding `Tasks` to the `Pipeline`
 ...
 Your `Pipeline` definition must reference at least one [`Task`](/vault/pipelines-v0.11.3/tasks).
...
You can also provide [`Parameters`](/vault/pipelines-v0.11.3/tasks#specifying-parameters):
...
allows you to list a series of references to [`Conditions`](/vault/pipelines-v0.11.3/./conditions) that are run before the task
...
In this example, `my-condition` refers to a [Condition](/vault/pipelines-v0.11.3/conditions) custom resource. The `build-push`
...
**Note:** If you do not specify a `Timeout` value, Tekton instead honors the timeout for the [`PipelineRun`](/vault/pipelines-v0.11.3/PipelineRun#syntax).
...
Tasks can emit [`Results`](/vault/pipelines-v0.11.3/tasks#storing-execution-results) while they execute. You can
use these `Results` values as parameter values in subsequent `Tasks` within your `Pipeline`
through [variable substitution](/vault/pipelines-v0.11.3/variables#variables-available-in-a-pipeline). Tekton infers the
...
- [`from`](#using-the-from-parameter) clauses on the [`PipelineResources`](/vault/pipelines-v0.11.3/resources) used by each `Task`, and
