...
To create a `TaskRun`, you must first create a [`Task`](/vault/pipelines-v0.11.3/tasks) which
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`taskRef` or `taskSpec`](#specifying-a-task) - Specifies the details of
      the [`Task`](/vault/pipelines-v0.11.3/tasks) you want to run
- Optional:
  ...
  - [`podTemplate`](#pod-template) - Specifies a [pod template](/vault/pipelines-v0.11.3/./podtemplates) that will be used as the basis for the `Task` pod.
  - [`workspaces`](#workspaces) - Specify the actual volumes to use for the
    [workspaces](/vault/pipelines-v0.11.3/workspaces#declaring-workspaces-in-tasks) declared by a `Task`
...
Since a `TaskRun` is an invocation of a [`Task`](/vault/pipelines-v0.11.3/tasks), you must specify
what `Task` to invoke.
...
If a `Task` has [`parameters`](/vault/pipelines-v0.11.3/tasks#parameters), you can specify values for
...
If a `Task` requires [input resources](/vault/pipelines-v0.11.3/tasks#input-resources) or
[output resources](/vault/pipelines-v0.11.3/tasks#output-resources), they must be provided to run the
`Task`.

They can be provided via references to existing
[`PipelineResources`](/vault/pipelines-v0.11.3/resources):
...
The `paths` field can be used to [override the paths to a resource](/vault/pipelines-v0.11.3/./resources#overriding-where-resources-are-copied-from)
...
[`ServiceAccount`](/vault/pipelines-v0.11.3/./auth) reference topic.

## Pod Template

Specifies a [pod template](/vault/pipelines-v0.11.3/./podtemplates) configuration that will be used as the basis for the `Task` pod. This
...
`TaskRun`s see [workspaces.md](/vault/pipelines-v0.11.3/./workspaces#providing-workspaces-with-taskruns).

Tekton supports several different kinds of `Volume` in `Workspaces`. For a list of
the different kinds see the section on
[`VolumeSources` for Workspaces](/vault/pipelines-v0.11.3/workspaces#volumesources-for-workspaces).
...
[`TaskRuns`](/vault/pipelines-v0.11.3/taskruns) (including referenced
[`PipelineResources`](/vault/pipelines-v0.11.3/resources)) instantiating the
[`Task` (`dockerfile-build-and-push`) in the `Task` example docs](/vault/pipelines-v0.11.3/tasks#example-task).
