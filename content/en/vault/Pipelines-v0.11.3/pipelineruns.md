...
On its own, a [`Pipeline`](/vault/pipelines-v0.11.3/pipelines) declares what [`Tasks`](/vault/pipelines-v0.11.3/tasks) to
run, and [the order they run in](/vault/pipelines-v0.11.3/pipelines#ordering). To execute the `Tasks`
in the `Pipeline`, you must create a `PipelineRun`.

Creation of a `PipelineRun` will trigger the creation of
[`TaskRuns`](/vault/pipelines-v0.11.3/taskruns) for each `Task` in your pipeline.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifiying-a-pipeline) - Specifies the [`Pipeline`](/vault/pipelines-v0.11.3/pipelines) you want to run.
- Optional:
  - [`resources`](#resources) - Specifies which
    [`PipelineResources`](/vault/pipelines-v0.11.3/resources) to use for this `PipelineRun`.
  ...
  - `timeout` - Specifies timeout after which the `PipelineRun` will fail. If the value of
    ...
    follow the instruction [here](/vault/pipelines-v0.11.3/taskruns#Configuring-default-timeout) to configure the
...
Since a `PipelineRun` is an invocation of a [`Pipeline`](/vault/pipelines-v0.11.3/pipelines), you must specify
...
When running a [`Pipeline`](/vault/pipelines-v0.11.3/pipelines), you will need to specify the
[`PipelineResources`](/vault/pipelines-v0.11.3/resources) to use with it. One `Pipeline` may need to
...
`PipelineRun`s see [workspaces.md](/vault/pipelines-v0.11.3/./workspaces#providing-workspaces-with-pipelineruns).
