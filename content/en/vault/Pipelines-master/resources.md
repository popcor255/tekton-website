...
used as inputs to a [`Task`](/vault/pipelines-master/tasks) and can be output by a `Task`.
...
> [the v1alpha1 to v1beta1 migration guide](/vault/pipelines-master/./migrating-v1alpha1-to-v1beta1#pipelineresources-and-catalog-tasks)
...
Resources can be used in [Tasks](/vault/pipelines-master/./tasks) and
[Conditions](/vault/pipelines-master/./conditions#resources).
...
at can be
[overridden with the `targetPath` field](/vault/pipelines-master/./resources#controlling-where-resources-are-mounted).
...
usually used as [a `Task` `output`](/vault/pipelines-master/tasks#outputs) for `Tasks` that build
...
[variable substitution](/vault/pipelines-master/tasks#variable-substitution):
...
[TaskRun Service Account](/vault/pipelines-master/./taskruns#service-account) can not be configured
