...
- Optional:
  ...
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/pipelines-master/resources) needed or created by your`Task`.
...
- The container image must abide by the [container contract](/vault/pipelines-master/./container-contract).
...
* `/workspace` - This directory is where [resources](#resources) and [workspaces](#workspaces)
  are mounted. Paths to these are available to `Task` authors via [variable substitution](/vault/pipelines-master/variables)
* `/tekton` - This directory is used for Tekton specific functionality:
    * `/tekton/results` is where [results](#results) are written to.
      The path is available to `Task` authors via [`$(results.name.path)`](/vault/pipelines-master/variables)
    * There are other subfolders which are [implementation details of Tekton](/vault/pipelines-master/developers/README#reserved-directories)
...
a [`PipelineResources`](/vault/pipelines-master/resources#using-resources) entity.
...
[`Workspaces`](/vault/pipelines-master/workspaces#using-workspaces-in-tasks) allow you to specify
...
For more information, see [Using `Workspaces` in `Tasks`](/vault/pipelines-master/workspaces#using-workspaces-in-tasks)
...
The stored results can be used [at the `Task` level](/vault/pipelines-master/./pipelines#configuring-execution-results-at-the-task-level)
or [at the `Pipeline` level](/vault/pipelines-master/./pipelines#configuring-execution-results-at-the-pipeline-level).
...
- [Mount a Kubernetes `Secret`](/vault/pipelines-master/auth).
...
For further information, see [`Sidecars` in `TaskRuns`](/vault/pipelines-master/taskruns#sidecars).
...
Also see the [complete list of variable substitutions for Tasks](/vault/pipelines-master/./variables#variables-available-in-a-task).
...
- To reference a parameter in a `Task`, use the following syntax, where `<name>` is the name of the parameter:
  ...
  $(params.<name>)
  ```
- To access parameter values from resources, see [variable substitution](/vault/pipelines-master/resources#variable-substitution)
