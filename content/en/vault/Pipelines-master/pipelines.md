...
- Optional:
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/pipelines-master/resources) needed or created by the `Tasks` comprising the `Pipeline`.
  - [`tasks`](#adding-tasks-to-the-pipeline):
      - `resources.inputs` / `resource.outputs`
        - [`from`](#using-the-from-parameter) - Indicates the data for a [`PipelineResource`](/vault/pipelines-master/resources)
...
- [Using `Workspaces` in `Pipelines`](/vault/pipelines-master/workspaces#using-workspaces-in-pipelines)
...
by using [variable substitution](/vault/pipelines-master/variables#variables-available-in-a-pipeline).
...
## Adding `Tasks` to the `Pipeline`
 ...
 Your `Pipeline` definition must reference at least one [`Task`](/vault/pipelines-master/tasks).
...
You can also provide [`Parameters`](/vault/pipelines-master/tasks#specifying-parameters):
...
[`Condition`](/vault/pipelines-master/./conditions) resources. The declared `Conditions` are run before the `Task` is run.
...
In this example, `is-master-branch` refers to a [Condition](/vault/pipelines-master/conditions) resource. The `deploy`
...
**Note:** If you do not specify a `Timeout` value, Tekton instead honors the timeout for the [`PipelineRun`](/vault/pipelines-master/pipelineruns#configuring-a-pipelinerun).
...
Tasks can emit [`Results`](/vault/pipelines-master/tasks#storing-execution-results) while they execute. You can
use these `Results` values as parameter values in subsequent `Tasks` within your `Pipeline`
through [variable substitution](/vault/pipelines-master/variables#variables-available-in-a-pipeline). Tekton infers the
...
- [`from`](#using-the-from-parameter) clauses on the [`PipelineResources`](/vault/pipelines-master/resources) used by each `Task`, and
