...
- [Events](/vault/pipelines-master/events#pipelineruns)



## Overview

A `PipelineRun` allows you to instantiate and execute a [`Pipeline`](/vault/pipelines-master/pipelines) on-cluster.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifying-the-target-pipeline) - Specifies the target [`Pipeline`](/vault/pipelines-master/pipelines).
- Optional:
  - [`resources`](#specifying-resources) - Specifies the [`PipelineResources`](/vault/pipelines-master/resources) to provision
  ...
  - [`taskRunSpec`](#specifying-task-run-specs) - Specifies a list of `PipelineRunTaskSpec` which allows for setting `ServiceAccountName` and [`Pod` template](/vault/pipelines-master/./podtemplates) for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  - [`timeout`](#configuring-a-failure-timeout) - Specifies the timeout before the `PipelineRun` fails.
  - [`podTemplate`](#pod-template) - Specifies a [`Pod` template](/vault/pipelines-master/./podtemplates) to use as the basis
...
A `Pipeline` requires [`PipelineResources`](/vault/pipelines-master/resources) to provide inputs and store outputs
...
wide `ServiceAccountName`  and [`podTemplate`](/vault/pipelines-master/./podtemplates) configuration,
for example:
...
- For information on mapping `Workspaces` to `Volumes`, see [Specifying `Workspaces` in `PipelineRuns`](/vault/pipelines-master/workspaces#specifying-workspaces-in-pipelineruns).
