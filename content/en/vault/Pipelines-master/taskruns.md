...
- [Events](/vault/pipelines-master/events#taskruns)
...
A `TaskRun` allows you to instantiate and execute a [`Task`](/vault/pipelines-master/tasks) on-cluster. A `Task` specifies one or more
...
- Optional:
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/pipelines-master/podtemplates) to use as
  ...
  - [`workspaces`](#specifying-workspaces) - Specifies the physical volumes to use for the
    [`Workspaces`](/vault/pipelines-master/workspaces#using-workspaces-in-tasks) declared by a `Task`.
...
If a `Task` has [`parameters`](/vault/pipelines-master/tasks#parameters), you can use the `params` field to specify their values:
...
If a `Task` requires [`Resources`](/vault/pipelines-master/tasks#specifying-resources) (that is, `inputs` and `outputs`) you must
specify them in your `TaskRun` definition. You can specify `Resources` by reference to existing
[`PipelineResource` objects](/vault/pipelines-master/resources) or embed their definitions directly in the `TaskRun`.
...
**Note:** You can use the `paths` field to [override the paths to a `Resource`](/vault/pipelines-master/resources#overriding-where-resources-are-copied-from).

### Specifying a `Pod` template

You can specify a [`Pod` template](/vault/pipelines-master/podtemplates) configuration that will serve as the configuration starting
...
- For information mapping `Workspaces` to `Volumes`, see [Using `Workspace` variables in `TaskRuns`](/vault/pipelines-master/workspaces#using-workspace-variables-in-taskruns).
- For a list of supported `Volume` types, see [Specifying `VolumeSources` in `Workspaces`](/vault/pipelines-master/workspaces#specifying-volumesources-in-workspaces).
...
For more information, see [`ServiceAccount`](/vault/pipelines-master/auth).
...
See [Building and pushing a Docker image](/vault/pipelines-master/tasks#building-and-pushing-a-docker-image)
